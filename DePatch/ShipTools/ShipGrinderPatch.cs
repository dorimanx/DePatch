...
namespace DePatch
{
	...
	internal class ShipGrinderPatch
	{
		...
		private static void Prefix(MyShipGrinder __instance)
		{
			...
			BoundingSphere boundingSphere = (BoundingSphere)ShipGrinderPatch.m_detectorSphere.GetValue((object) __instance);
			...
			if (list.Contains((MyEntity) __instance.CubeGrid))
				list.Remove((MyEntity) __instance.CubeGrid);
			...
			foreach (MyEntity myEntity in list)
			{
				...
				if (myCubeGrid != null && myEntity.Physics != null)
				{
					...
					float grinderAmount = MySession.Static.WelderSpeedMultiplier * enumerable.First<ShipTool>().Speed;
					ShipGrinderPatch.slimBlocks.Clear();
					myCubeGrid.GetBlocksInsideSphere(ref boundingSphereD, ShipGrinderPatch.slimBlocks, true);
					foreach (MySlimBlock mySlimBlock in ShipGrinderPatch.slimBlocks)
