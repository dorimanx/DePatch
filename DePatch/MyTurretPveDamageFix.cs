	...
	internal class MyTurretPveDamageFix
	{
		private static bool Prefix(MySessionComponentSafeZones __instance, MyEntity entity, MySafeZoneAction action, ref bool __result)
		{
			...
			else
			{
				...
				if (character != null && action == MySafeZoneAction.Shooting)
				{
					MyPlayer myPlayer = MySession.Static.Players.GetOnlinePlayers().ToList<MyPlayer>().Find((MyPlayer b) => b.Identity.IdentityId == character.GetPlayerIdentityId());
