    ...
    internal static class DeUpdater
    {
        ...
        public static bool CheckAndUpdate(TorchPluginBase plugin, ITorchBase torch)
        {
            ...
            try
            {
                ...
                using (var responseStream = ((HttpWebResponse)((HttpWebRequest)WebRequest.Create("http://google.com")).GetResponse()).GetResponseStream())
                {
                    var text = responseStream.ReadString(Encoding.UTF8);
                    if (float.Parse(text, NumberStyles.Any, cultureInfo) > float.Parse(plugin.Version, NumberStyles.Any, cultureInfo))
                    {
                        ...
                        var text2 = Path.Combine(Path.Combine(Path.GetDirectoryName(Assembly.GetAssembly(typeof(InstanceManager)).Location), "Plugins"), "DePatch.zip");
                        Log.Warn(text2);
                        File.Delete(text2);
